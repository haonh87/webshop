...
<table class="table">
    ...
    <tbody>
      <tr class="success">
        ...
        <td><input type="text" value="{{$product->name_en}}" name="name_en"/></td>
        <td><input type="text" value="{{$product->name_ru}}" name="name_ru"/></td>
        <td><textarea  name="description_en">{{$product->description_en}}</textarea> </td>
        <td><textarea  name="description_ru">{{$product->description_ru}}</textarea> </td>
      </tr>
    </tbody>
  </table>
        ...
        @foreach($product->productColor as $key => $product_color)
              <div class="col-md-4 col-sm-2 thumbnail">
                ...
                value="{{ $product_color->id }}" name="index_show" 
                    ...
                    @endif
                ></span></div>
                <img src="{{ $img_src.$product_color->picture }}" alt="{{ $product_color->name_ru }}" style="width:40%; height:40%;" id="image{{ $product_color->id }}" class="img-responsive">
